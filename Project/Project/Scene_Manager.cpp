	...
	else
	{
		...
		if (nScene_Id == 2)
		{
			...
			// Fireball - 4
			{
				...
				tScene.fWorld_Matrix[4][0] = XMFLOAT4x4_to_tFloat4x4(temp);
				...
				tScene.tMaterials_Data[4][0] = cBinary_Read.Read_Material("fireballMaterial.bin");
				tScene.tMaterials_Data[4][0].tMats[0].szDiffuse_File_Path = "Fireball.fbm\\Fireball_D.png";
				//tScene.tMaterials_Data[0].tMats[0].tNormal.fX = 1.0f;
			}
			// Fireball
			...
			// Priest - 4
			{
				...
				tempMatrix = XMMatrixMultiply(XMMatrixTranslation(0, 5, -100), tempMatrix);
				...
				tScene.bHas_Animation[5] = true;
				tScene.tAnim_Clip[5] = cBinary_Read.Read_Skeleton("priestSkeleton.bin");
				tScene.tAnim_Data[5] = Create_Inverse_Bind_Pose(tScene.tAnim_Clip[5].tKeyFrames[0]);
			}
			// Priest
