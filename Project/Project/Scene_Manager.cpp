	...
	else
	{
		...
		*/
		...
		// Anim Test - 2
		{
			...
			XMMATRIX tempMatrix = XMMatrixIdentity();
			tempMatrix = XMMatrixMultiply(XMMatrixTranslation(-20, 8, 0), tempMatrix);
			...
			tMesh_Skinned tDragon = cBinary_Read.Read_Mesh_Skinned("Test_Cylinder_10bonesSkinned.bin");
			...
			tScene.tMaterials_Data[2] = cBinary_Read.Read_Material("Test_Cylinder_10bonesMaterial.bin");
			tScene.tMaterials_Data[2].tMats[0].szDiffuse_File_Path = "Dragon.fbm\\DarkDragon_D.png";

			tScene.tAnim_Clip[2] = cBinary_Read.Read_Skeleton("Test_Cylinder_10bonesSkeleton.bin");
